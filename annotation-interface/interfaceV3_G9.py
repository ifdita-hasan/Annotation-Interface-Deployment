...
def update_global_dict(keys, dump = False):
    ...
    if "logged_in" in st.session_state and st.session_state["logged_in"]:
        if save_on_cloud:
            save_dict_to_gcs(BUCKET_NAME, f"data/state_eval_g9_{st.session_state['logged_in']}.json", global_dict)
        else:
            json.dump(global_dict, open(f"data/state_eval_g9_{st.session_state['logged_in']}.json", 'w'))
    elif "pid" in st.session_state and st.session_state["pid"]:
        if save_on_cloud:
            client = get_gc_client()
            bucket = client.get_bucket(BUCKET_NAME)
            if storage.Blob(bucket=bucket, name=f"data/state_eval_g9_{st.session_state['pid']}.json").exists(client):
                return
        else:
            if os.path.exists(f"data/state_eval_g9_{st.session_state['pid']}.json"):
                # load
                return
        if save_on_cloud:
            save_dict_to_gcs(BUCKET_NAME, f"data/state_eval_g9_{st.session_state['pid']}.json", global_dict)
        else:
            json.dump(global_dict, open(f"data/state_eval_g9_{st.session_state['pid']}.json", 'w'))
    else:
        if save_on_cloud:
...
def example_finished_callback():
    ...
    if "logged_in" in st.session_state and st.session_state["logged_in"]:
        if save_on_cloud:
            ...
            json.dump(dict(global_dict), open(f"data/state_eval_g9_{st.session_state['logged_in']}.json", 'w'))
    else:
        if save_on_cloud:
            save_dict_to_gcs(BUCKET_NAME, f"data/state_eval.json", global_dict)
        else:
...
if __name__ == "__main__":
    ...
    if "reload" not in st.session_state or st.session_state["reload"]:
        if "logged_in" in st.session_state and st.session_state["logged_in"]:
            if save_on_cloud:
                global_dict = read_or_create_json_from_gcs(BUCKET_NAME, f"data/state_eval_g9_{st.session_state['logged_in']}.json")
            else:
                global_dict = json.load(open(f"data/state_eval_g9_{st.session_state['logged_in']}.json", 'r'))
        elif "pid" in st.session_state and st.session_state["pid"]:
            if save_on_cloud:
                global_dict = read_or_create_json_from_gcs(BUCKET_NAME, f"data/state_eval_g9_{st.session_state['pid']}.json")
            else:
                global_dict = json.load(open(f"data/state_eval_g9_{st.session_state['pid']}.json", 'r'))
        else:
            if save_on_cloud:
