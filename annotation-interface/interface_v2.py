...
# """)
        ...
        with c1.container(height=1000):
            if example_ind >= len(global_dict["testcases"]):
                ...
                st.markdown(f"""Instructions: Below is a :blue[conversation context] for psycotherapy conversation {c_index}. 
        ...
        with c2.container(height=1000):
            if example_ind >= len(global_dict["testcases"]):
                ...
                with st.container():
                    ...
                    st.markdown(f"""Select below which areas need improvement and select the subareas accordingly""")
                    main_options = ["None", "Questions", "Suggestions"]
                    subskill_options = [["Is the helper asking questions that are too focused with closed-questions instead of exploring with open-ended questions?", 
                    ...
                    rating_scale = ["Select an option", "Yes", "No"]
                    select_key = f"skill{example_ind}"
                    count_required_feedback += 1
                    # print("session state key:", select_key)
                    ...
                    if "None" not in selected_main_option:
                        with st.container():
                            ...
                            for j, option in enumerate(subskills):
                                count_required_feedback += 1
                                radio_key = f"subskill_{j}_{example_ind}"
                                st.write(f"{option}")
                                default_value = " " if subskills else "None"
                                selection = st.radio(" ", rating_scale, index=0, kwargs = (radio_key), key= radio_key)
