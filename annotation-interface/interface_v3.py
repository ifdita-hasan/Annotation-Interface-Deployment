...
def update_global_dict(keys, dump = False):
    ...
    if "logged_in" in st.session_state and st.session_state["logged_in"]:
        ...
        if save_on_cloud:
            save_dict_to_gcs(BUCKET_NAME, f"data/state_eval_g1_.json", global_dict)
        else:
            json.dump(global_dict, open(f'data/state_eval_g1_.json', 'w'))
...
def example_finished_callback():
    ...
    if "logged_in" in st.session_state and st.session_state["logged_in"]:
        ...
        if save_on_cloud:
            ...
            json.dump(dict(global_dict), open(f'data/state_eval_g1_.json', 'w'))
...
if __name__ == "__main__":
    ...
    if "reload" not in st.session_state or st.session_state["reload"]:
        if "logged_in" in st.session_state and st.session_state["logged_in"]:
            ...
            if save_on_cloud:
                global_dict = read_or_create_json_from_gcs(BUCKET_NAME, f"data/state_eval_g1_.json")
            else:
                global_dict = json.load(open(f'data/state_eval_g1_.json', 'r'))
